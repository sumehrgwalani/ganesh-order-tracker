...
export function orderToPdfData(order: any, meta: Record<string, any> | null, lineItems: any[]): PdfData {
  ...
  return {
    ...
    supplierAddress: meta?.supplierAddress || order.supplier_address || '',
    supplierCountry: (meta?.supplierCountry || order.from || order.from_location || 'India').toUpperCase(),
    buyerName: meta?.buyer || order.company,
    buyerBank: meta?.buyerBank || order.buyer_bank || '',
    destination: meta?.destination || order.to || order.to_location || '',
    ...
    overseasCommission: meta?.overseasCommission || order.overseas_commission || '',
