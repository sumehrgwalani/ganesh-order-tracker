...
[{ "gmail_id": "EXACT_ID_FROM_ABOVE", "matched_order_id": "PO-NUMBER or null", "detected_stage": 3 or null, "summary": "What THIS specific email is about" }]`
    ...
    if (syncMode === 'reprocess') {
      ...
      // Find matched emails with attachments where AI detected stage 1 or 2
      const { data: emails, error: fetchErr } = await supabase
        .from('synced_emails')
        .select('id, gmail_id, subject, from_name, from_email, date, has_attachment, matched_order_id, ai_stage')
        .eq('organization_id', organization_id)
        .not('matched_order_id', 'is', null)
      ...
      for (const email of emails) {
        ...
        try {
          ...
          processed++
          results.push({ order: email.matched_order_id, stage: email.ai_stage, status: 'ok' })
        } catch (err) {
          results.push({ order: email.matched_order_id, stage: email.ai_stage, status: 'error', error: String(err) })
        }
      ...
      return new Response(JSON.stringify({
        ...
        processed,
        poAttachments: poCount,
        piAttachments: piCount,
        results,
      }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
